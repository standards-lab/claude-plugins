...
finished, and you want a fresh context to keep going on the same step and the same branch. `reset` is
the pipeline's CONCLUDE stage taken mid-work (`mechanics/pipeline.md`).
...
don't publish it — the work isn't finished. A later session will see `Status: handoff`, check out the
