...
- **Decay** — if the code now expresses a `design/` note, delete that note.
- **Cull** — drop a `concepts/` candidate this session's work has invalidated or replaced.
- **Promote** — move a `concepts/` candidate into `design/` if it proved out, and say why.
...
Optionally make a WIP commit on the branch so in-progress code isn't lost. Leave the branch open and
