...
Read the `context/` notes against the code and look for:

- **Decay** — `design/` notes the code now fully expresses. These should go; the design is describing
  the present instead of the unbuilt.
- **Cull** — `concepts/` candidates the implementation invalidated or replaced, or that have just gone
  stale.
...
the decayed notes (recording that you did), cull the dead candidates, promote the proven ones, and fix
the orientation.
