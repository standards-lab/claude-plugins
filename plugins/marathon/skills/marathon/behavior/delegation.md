...
- **editor** — edits the prose a branch changed, once, after the last stage commits and before
  the final checkpoint. It edits in place and never changes meaning.
...
- The grain is one unit: one stage list, one stage, one branch edit, or one branch review. A
  delegate that reaches past its unit is a finding for a re-plan.
- A delegate never commits, publishes, or writes the reset file, and changes nothing at SETTLE.
  The session reads any prose a delegate writes before the repository keeps it.
